...
/**
 * Default items for the "Incharge ประจำวัน" daily checklist, seeded only the
 * very first time the server runs (mirrors seedTasks' first-run pattern).
 * period: 'morning' | 'midday' | 'evening'.
 * frequency: 'daily' (every day) or 'custom' (only on the weekdays listed in
 * `days`, where 0=Sunday ... 6=Saturday — matches JS Date#getDay()).
 */
function seedChecklistItems() {
  function uid() {
    return 'c' + Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
  }
  function item(text, period, frequency, days) {
    return {
      id: uid(),
      text,
      period,
  ...
  return [
    item('Morning brief ประจำวัน', 'morning', 'daily'),
    item('เช็คใบเสร็จ', 'morning', 'daily'),
    item('ทำเอกสารปะหน้าใบเสร็จ', 'morning', 'daily'),
    item('เช็คใบเช็คชื่อ', 'morning', 'daily'),
    item('ทำเอกสารปะหน้าใบเซ็นชื่อ', 'morning', 'daily'),
    item('สแกนเอกสารใบเสร็จและใบปะหน้า ส่งอีเมลให้พี่ตาล', 'morning', 'daily'),
    item('ส่งเอกสารไปบริษัท', 'midday', 'custom', [4]), // 4 = Thursday
    item('CF แพทย์', 'evening', 'daily'),
    item('CF part time', 'evening', 'daily'),
  ];
}

module.exports = { seedTasks, seedChecklistItems };
